...
const Job = ({ job }) => {
  ...
  return (
    <div className="job-card">
      ...
      <div className="job-card-title">{job.url}</div>
      <div className="job-card-subtitle">{job.description.slice(1, 100)}</div>
      <div className="job-detail-buttons">
        <button className="search-buttons detail-button">{job.job_type}</button>
      ...
      {show ? <SingleJob job={job.url} /> : null}
    </div>
  );
};
